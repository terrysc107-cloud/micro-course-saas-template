      ...
      <div className="relative max-w-6xl mx-auto px-6 pt-16 pb-16 sm:pt-24 sm:pb-20">
        <div className="max-w-3xl">
          ...
          <p className="text-lg sm:text-xl text-slate-400 leading-relaxed mb-8 text-pretty">
            A few narrow assistants that read your real numbers on a schedule and hand you
            a meeting you can act on. You have never written code, and you will not start here.
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <BuyButton
              ...
              showChevron
              className="bg-gold hover:brightness-95 text-slate-50 px-7 py-4 rounded-xl text-base sm:text-lg shadow-[0_0_28px_rgba(201,168,76,0.28)] hover:shadow-[0_0_36px_rgba(201,168,76,0.42)] transition-shadow"
            />
            <Link
              href="#curriculum"
              className="inline-flex items-center justify-center gap-2 border border-slate-700 hover:border-slate-500 hover:bg-slate-900/50 text-slate-300 hover:text-slate-50 px-7 py-4 rounded-xl text-base sm:text-lg font-semibold transition-colors"
            >
              See the curriculum
            </Link>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4 max-w-lg mt-14">
          {stats.map(({ icon: Icon, val, label }) => (
            ...
            >
              ...
              <p className="text-2xl font-bold text-slate-50 tabular-nums">{val}</p>
